  ...
  return (
    <div className="flex flex-col h-[calc(100vh-220px)]">
      <div className="mb-3 text-sm text-[hsl(var(--muted-foreground))]">
        Asking as <span className="text-indigo-700 font-medium">{labelPersona(persona)}</span>
        {' · '}region <span className="text-indigo-700 font-medium">{region}</span>
      </div>
      ...
      >
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {EXAMPLES.map((q) => (
                <button
                  key={q}
                  onClick={() => ask(q)}
                  className="text-sm rounded-full border border-[hsl(var(--border))] px-3 py-1.5 hover:border-indigo-700 hover:text-indigo-700 transition-colors"
                >
                  {q}
                </button>
              ))}
        ...
        {pending && (
          <div className="flex items-center gap-2 text-sm text-[hsl(var(--muted-foreground))] fade-in">
            <span className="inline-block size-2 rounded-full bg-indigo-700 animate-pulse" />
      ...
      >
        <input
          ...
          disabled={pending}
          className="flex-1 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600/60"
        />
        <button
          type="submit"
          disabled={pending || !input.trim()}
          className="rounded-md bg-indigo-700 hover:bg-indigo-800 disabled:opacity-50 disabled:cursor-not-allowed px-4 py-2 text-sm font-medium text-white"
        >
          Ask
        </button>
      </form>
...
function UserBubble({ text }: { text: string }) {
  return (
    <div className="flex justify-end fade-in">
      <div className="max-w-[80%] rounded-2xl bg-indigo-600/15 border border-indigo-600/30 px-4 py-2 text-sm">
        {text}
      </div>
    </div>
  )
          ...
          {m.citations.map((id) => {
            ...
            return (
              <button
                ...
                className="text-xs font-mono rounded-full border border-indigo-600/50 bg-indigo-600/10 hover:bg-indigo-600/20 px-2 py-1 text-indigo-700"
              >
                [{id}]
              </button>
            )
...
function renderInline(text: string): React.ReactNode[] {
  ...
  for (const m of text.matchAll(pattern)) {
    ...
    if (tok.startsWith('**')) {
      ...
      out.push(
        <span key={key++} className="font-mono text-indigo-700">
          {tok}
        </span>,
      )
    }
...
function Trace({
  ...
  return (
    <details className="group rounded-lg border border-indigo-600/30 bg-indigo-600/5">
      <summary className="cursor-pointer select-none list-none px-3 py-2 text-xs uppercase tracking-wider text-indigo-700/90 flex items-center gap-2">
      ...
      <div className="px-3 pb-3 pt-1 text-xs space-y-3">
        <section>
          <div className="font-semibold text-indigo-700/80 mb-1">Retrieved</div>
              ...
              <li key={c.id} className="flex items-center gap-2">
                ...
                <span
                  className={cn(
                    ...
                    c.reason === 'min_persona' && 'bg-red-500/15 text-red-700',
                    c.reason === 'deprecated' && 'bg-amber-500/15 text-amber-700',
                    c.reason === 'region' && 'bg-blue-500/15 text-blue-700',
                  )}
                >
                  {c.reason}
                </span>
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-mono text-sm text-indigo-700">[{chunk.document_id}]</h3>
          <button
            onClick={onClose}
